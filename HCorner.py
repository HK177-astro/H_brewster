    ...
    **hist2d_kwargs,
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            ...
            n, hist_edge, patches_hist_1 = ax.hist(x, bins=bins_1d, weights=weights, **hist_kwargs)

            #bins_mod = np.histogram_bin_edges(x, bins='auto')

            #n, hist_edge, _ = ax.hist(x, bins=bins_mod, weights=weights, **hist_kwargs)

            hist_samples.append(n)
            bin_edges_1.append(hist_edge)
            patches_hist.append(patches_hist_1)

            """n, bin_edges, _ = ax.hist(x, bins=bins_1d, weights=weights, **hist_kwargs)
            for i in range(len(bin_edges) - 1):
                color = 'r' if bin_edges[i] <= quantiles[0] else ('g' if bin_edges[i] <= quantiles[1] else 'b')
                plt.plot([bin_edges[i], bin_edges[i + 1]], [n[i], n[i]], color=color, linestyle='-', linewidth=2)
                if i < len(bin_edges) - 2:  # Ensure not to exceed the array length
                    plt.plot([bin_edges[i + 1], bin_edges[i + 1]], [n[i], n[i + 1]], color=color, linestyle='-', linewidth=2)"""
    ...
    if truths is not None:
        overplot_lines(axes, truths, reverse=reverse, color=truth_color)
        overplot_points(
            axes,
    ...
    return (fig, axes), hist_samples, bin_edges_1, patches_hist
...
def overplot_points(axes, xs, reverse=False, **kwargs):
    """
...
def arviz_corner(
    ...
    **hist2d_kwargs,
    ...
    fig, hist_samples, bin_edges_1, patches_hist = corner_impl(
        samples,
        bins=bins,
        range=range,
        axes_scale=axes_scale,
    ...
    return fig, hist_samples, bin_edges_1, patches_hist